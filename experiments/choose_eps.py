...
for DELTA in DELTAS:
    for SIZE_PATTERN in tqdm(SIZE_PATTERNS):
        for SCALER in SCALER_RANGE:
            ...
            for skip in tqdm(range(1, test.shape[1], 5), leave=False):
                # Обработка тестового датасета со сдвигом на skip шагов
                if 3 + skip + DELTA + np.sum(current_pattern) + 1 >= test.shape[1]:
                    break
                data, idxs1, idxs2  = cut_ts(data=test, pattern=current_pattern, delta=DELTA, skip=skip, detrended=True)
                if data.shape[0] == 0:
                    break
                data = scale(data=data, scaler=SCALER)
