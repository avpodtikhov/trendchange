# Фильтрация списка паттернов в обратную сторону от отступа
import pandas as pd
import os
import re
import numpy as np
...
for DELTA in tqdm(DELTAS):
    for SIZE_PATTERN in tqdm(SIZE_PATTERNS, leave=False):
        ...
        for eps in [SIZE_TO_EPS[SIZE_PATTERN]]:
            patterns = pd.read_csv(f'/home/avpodtikhov/trendchange/results/hun_zer/{SIZE_PATTERN}/{DELTA}.csv')
            for skip in tqdm(range(-1, -(DELTA + 1), -1), leave=False):
                ...
                path = os.path.join('/home/avpodtikhov/trendchange/results/hun_zer/clean/', '{}'.format(SIZE_PATTERN), '{}.csv'.format(DELTA))
            ...
            if stop:
                break
            for skip in tqdm(range(1, 30), leave=False):
                # Обработка тестового датасета со сдвигом на skip шагов
                data  = cut_ts(data=test, pattern=current_pattern, delta=DELTA, skip=skip)
                data = scale(data=data, scaler=SCALER)
                # Считаем число ложных срабатываний паттернов
